...
export default function BlogPost2(){
  		...
  		if(searchParams.get('ema3')){
  		 ...
  		 if(searchParams.get('end')){
  		 	...
  		 	p2 = searchParams.get('end');
				...
				fetch(`https://functions.yandexcloud.net/d4en7mmofqr0fh08od00?start=${p1}&end=${p2}&ema1=${p3}&ema2=${p4}&ema3=${p5}`)
	...
	function sendRequest(e){
		...
		const url = `https://functions.yandexcloud.net/d4en7mmofqr0fh08od00?start=${date1}&end=${date2}&ema1=${ema1}&ema2=${ema2}&ema3=${ema3}`;
...
//https://jaywilz.github.io/react-bootstrap-range-slider/
	...
	return <>
	<Container>
	<Row><h1>Hedging ARKW ETF with ARKK</h1></Row>
    <br/>
...
<Col lg={5}>
    <p>This blog post is a slight variation on the previos blog post about <Link to="../arkk-momentum">hedging ARKK ETF</Link>. 
In that post I described a trading strategy when you exit your long position in ARKK whenever the trend turns down. 
In this post I present a slightly different trading strategy where you go <b>long ARKW ETF (Next Generation Internet ETF) and hedge this long position with ARKK</b> whenever the trend turns down. 
...
The two ETFs hold a slightly different set of stocks with different weights, the correlation between ARKW and ARKK is 0.93. 
The high correlation is good for hedging.
    </p>

        <p><b>In the form below perform your backtest</b> by choosing the parameters of EMAs and the backtest period. 
    The output contains the comparison of a simple long and hedging. 
    The EMAs determine if the trend in ARKK ETF is down. 
    If the trend is down, the backtest hedges the long ARKW position by going short ARKK.</p>
